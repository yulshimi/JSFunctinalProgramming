...
const ls_size = ls => q.fold_left((acc) => acc+1, 0, ls);
...
const sum_sqrs = ls => q.fold_left((base, acc) => base + acc*acc, 0, ls);
...
const ls_min = ls => q.fold_left((base, acc) => {
  if(acc < base){
    base = acc;
  }
  return base;
}, ls.get(0), ls);
const ls_max = ls => q.fold_left((base, acc) => {
  if(base < acc){
    base = acc;
  }
  return base;
...
const ls_evens = ls => q.fold_left((base, acc) => {
  if(acc % 2 === 0){
    return base.push(acc);
  }
  return base;
}, List([]), ls);
const ls_odds  = ls => q.fold_left((base, acc) => {
  if(acc % 2 === 1){
    return base.push(acc);
  }
  return base;
