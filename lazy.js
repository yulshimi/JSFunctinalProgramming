...
function map(f, thunk_xs) {
  return delay(function() {
    return { 
      head: f(force(thunk_xs).head),
      tail: map(f, force(thunk_xs).tail) 
    };
  });
}
...
function zipWith(f, thunk_xs, thunk_ys) {
  return delay(function() {
    return { 
      head: f(force(thunk_xs).head, force(thunk_ys).head),
      tail: zipWith(f, force(thunk_xs).tail, force(thunk_ys).tail) 
    };
  });
}
